...
document.getElementById('game').addEventListener('keyup', ev => {
    ...
    console.log({keyPressed, expectedLetter, isLetter, isSpace, isBackspace, isFirstLetter});
    if (isLetter) {
        if (currentLetter) {
            addClass(currentLetter, keyPressed === expectedLetter ? 'correct' : 'incorrect')
    ...
    if (isBackspace) {
        if (currentLetter && isFirstLetter) {
            // make prev word current, prev word last letter current letter
            removeClass(currentWord, 'current');
            ...
            removeClass(currentLetter, 'current');
            addClass(currentLetter.previousLetter, 'current');
        }
    }
    ...
    if (nextLetter) {
        ...
        cursor.style.top = currentLetter.getBoundingClientRect().top + 5 + 'px';
